...
def _decimal(value, field):
    try:
        ...
        raise SalesValidationError(f"{field} must be a valid number")
    if number < 0:
        raise SalesValidationError(f"{field} must be non-negative")
    return number


def create_sale(*, organization_id, commercial_id, client_id=None, lines, status="confirmed", notes=None):
    if not lines:
        raise SalesValidationError("At least one sale line is required")
    commercial = Commercial.query.filter_by(id=commercial_id, organization_id=organization_id, deleted_at=None).first()
    if commercial is None:
        raise SalesValidationError("Commercial not found in current organization")
    client = None
    if client_id is not None:
        client = Client.query.filter_by(id=client_id, organization_id=organization_id, deleted_at=None).first()
        if client is None:
    ...
    sale = Sale(organization_id=organization_id, commercial=commercial, client=client, status=status, notes=notes)
    for item in lines:
        if not isinstance(item, dict):
            raise SalesValidationError("Each line must be an object")
        product_id = item.get("product_id")
        product = Product.query.filter_by(id=product_id, organization_id=organization_id, deleted_at=None, active=True).first()
        if product is None:
        ...
        line = SaleLine(organization_id=organization_id, product=product, quantity=quantity, unit_price=unit_price)
...
def set_sales_target(*, organization_id, commercial_id, year, month, target_amount):
    ...
    target = SalesTarget.query.filter_by(
        organization_id=organization_id, commercial_id=commercial_id, year=int(year), month=int(month)
    ).first()
    if target is None:
        target = SalesTarget(organization_id=organization_id, commercial_id=commercial_id, year=int(year), month=int(month))
