...
class StockTransfer(TenantAwareModel):
    ...
    source_store_id = db.Column(db.Integer, db.ForeignKey("store.id", ondelete="RESTRICT"), nullable=False, index=True)
    destination_store_id = db.Column(db.Integer, db.ForeignKey("store.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class StockTransferItem(TenantAwareModel):
    ...
    product_id = db.Column(db.Integer, db.ForeignKey("product.id", ondelete="RESTRICT"), nullable=False, index=True)
    quantity = db.Column(db.Numeric(14, 3), nullable=False)
    batch_id = db.Column(db.Integer, db.ForeignKey("product_batch.id", ondelete="RESTRICT"), nullable=True, index=True)
