...
class CashRegister(TenantAwareModel):
    ...
    store_id = db.Column(
        db.Integer,
        db.ForeignKey("store.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    name = db.Column(db.String(255), nullable=False)
    ...
    sessions = db.relationship(
        "CashSession", back_populates="register", cascade="all, delete-orphan"
    )
...
class CashSession(TenantAwareModel):
    ...
    register_id = db.Column(
        db.Integer,
        db.ForeignKey("cash_register.id", ondelete="RESTRICT"),
        nullable=False,
        index=True,
    )
    opened_by = db.Column(
