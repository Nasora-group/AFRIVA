...
def clients():
    rows = (
        Client.query.filter_by(
            organization_id=g.current_org_id, deleted_at=None
        )
...
def prospects():
    ...
    return jsonify(
        {"items": [_prospect_json(row) for row in rows], "count": len(rows)}
    )
...
def commercials():
    rows = (
        Commercial.query.filter_by(
            organization_id=g.current_org_id, deleted_at=None
        )
        .order_by(Commercial.last_name, Commercial.first_name)
        .all()
    )
    return jsonify(
        {
...
def prospections():
    rows = (
        Prospection.query.filter_by(organization_id=g.current_org_id)
        .order_by(Prospection.created_at.desc())
        .all()
    )
    return jsonify(
        {
            "items": [
                {
                    ...
                    "status": row.status,
                    "potential": float(row.potential or 0),
                }
                for row in rows
            ],
            "count": len(rows),
