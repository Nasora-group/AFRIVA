"""API endpoints for stock transfers and FEFO batch consumption."""
...
def create_transfer():
    ...
    try:
        transfer = InventoryService().create_transfer(
            source_store_id=int(payload["source_store_id"]),
            destination_store_id=int(payload["destination_store_id"]),
            items=payload["items"],
