...
def test_period_end_monthly():
    ...
    assert BillingService._period_end(start, "monthly") == datetime(2026, 9, 27, tzinfo=timezone.utc)


def test_period_end_yearly():
    start = datetime(2026, 8, 28, tzinfo=timezone.utc)
    assert BillingService._period_end(start, "yearly") == datetime(2027, 8, 28, tzinfo=timezone.utc)
...
def test_organization_id_requires_tenant_context(app, monkeypatch):
    monkeypatch.setattr("app.services.billing_service.get_current_organization", lambda: None)
    with app.app_context(), pytest.raises(ValueError, match="No current organization"):
...
def test_record_partial_then_full_payment_marks_invoice_paid(app, tenant):
    with app.app_context():
        ...
        first = service.record_payment(invoice.id, "4000", provider="wave", provider_reference="P1")
